...
if __name__ == '__main__':
    ...
    # -vdd=<value>  Supply voltage of the standard cell library (used to set
    #               adc_bridge / dac_bridge thresholds). GF180MCU digital
    #               libraries are commonly run at 3.3V or 1.8V depending on
    #               the corner; check the liberty file's nom_voltage and set
    #               this to match, since 3.0V (the qflow/osu035 default) is
    #               very likely wrong for GF180MCU.
    vdd = 3.0
    try:
        vddopt = next(item for item in options if item.startswith('-vdd='))
    except StopIteration:
        pass
    else:
        vdd = float(vddopt.split('=')[1])
